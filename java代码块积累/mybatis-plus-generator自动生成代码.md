...
修改包名、作者、路径、jdbc配置后，运行main()方法，输入表名即可，多个表用逗号隔开。

jdbc的配置。需要修改 数据库地址:端口/库名。useSSL如果为true，简单密码(如12345)无法通过验证。
路径。PATH 。修改为项目路径，一般是 /src/main/java 所在的目录。
...
public class MybatisResourcesGenerator {
    ...
    /**
     * 如果输出的路径不对， PATH 修改为项目路径，一般是 /src/main/java 所在的目录
     */
     ...
     * jdbc配置。自行修改。
     * 
    * 此处需要修改 数据库地址:端口/库名。useSSL如果为true，简单密码(如12345)无法通过验证。
     */
    
    private static final String JDBC_MYSQL = "jdbc:mysql://数据库地址:端口/库名?useUnicode=true&characterEncoding=utf-8&allowMultiQueries=true&useSSL=false&serverTimezone=Asia/Shanghai&useTimezone=true";
